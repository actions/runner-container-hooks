...
async function awaitRpcCompletion(url: string): Promise<RpcResult> {
  let status = await getRpcStatus(url)
  while (status.status !== 'completed' && status.status !== 'failed') {
    core.warning(`Waiting for completion (id = ${status.id})...`)
    await new Promise(resolve => setTimeout(resolve, 1000))
    status = await getRpcStatus(url)
  }
...
export async function rpcPodStep(
  ...
  await startRpc(url, id, containerPath).then(() =>
    awaitRpcCompletion(url).then(status =>
