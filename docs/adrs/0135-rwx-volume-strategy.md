...
The previous volume-less implementation, which copied the workspace into the job pod through the `exec` API, is not deleted. It is split out into its own `packages/k8s-novolume` package so that operators without any shared storage keep a supported option, while `packages/k8s` returns to relying on volumes. The two packages are released as separate archives, and only one of them is configured on a runner at a time.
...
- **Volume-less copy (`exec cp`):** Copying the workspace into the job pod removes the storage requirement entirely, but the `exec` API proved too slow for the directory structure and the amount of files involved, and it turned the hook into a filesystem driver. It was rejected as the default, and is kept as the separate `k8s-novolume` implementation.
